...
import useMenuService from '@/hooks/use-house';
...
const BillsList = ({ data }) => {
  ...
  const removeDuplicates = (arr) => {
    return arr.filter((item, index) => {
      return (
        arr.findIndex((t) => t.billdetailId === item.billdetailId) === index
      );
    });
  };
              ...
              <TableCell>
                <div
                  className={`font-bold px-1 rounded-md w-fit 
                                    ...
                                    ${
                                      item.payment.transactionStatus ===
                                        'returned' && 'bg-red-600 text-zinc-50'
                                    }
                                    ${
                                      item.payment.transactionStatus ===
                                        'pending' && 'bg-zinc-400 text-zinc-50'
                                    }
                                    ${
                                      item.payment.transactionStatus ===
                                        'ordered' && 'bg-sky-400 text-zinc-50'
                                    }
                                    `}
                >
                  {item.payment.transactionStatus}
